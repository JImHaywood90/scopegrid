...
export default function DashboardProducts() {
  ...
  const companyParam = sp.get('CompanyIdentifier') ?? sp.get('companyIdentifier') ?? null;
  ...
  const filtered = useMemo(() => {
    return items.filter((p) => {
      ...
      const hay = `${p.name} ${p.vendor ?? ''} ${p.category ?? ''}`.toLowerCase();
  ...
  return (
    <section className="flex-1 p-4 lg:p-8">
      ...
      {error ? (
        error.status === 400 ? (
          <div className="text-sm text-muted-foreground">Pick a company to view products.</div>
        ) : (
