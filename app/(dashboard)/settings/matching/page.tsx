...
const fetcher = async (u: string) => {
  ...
  try {
    return JSON.parse(t) as ScanResp;
  } catch {
    throw new Error(t || "Bad JSON");
  }
};

export default function MatchingPage() {
  const { identifier } = useCompanyContext();
  const { data, error, isLoading, mutate } = useSWR<ScanResp>(
    identifier
      ? `/api/matching/scan?companyIdentifier=${encodeURIComponent(identifier)}`
      : null,
    fetcher
  );
